 ...
 for iSat in range(len(satellite)):
  ...
  while DT != endDt:
     ...
     try:
         ...
         for iN in range(n1,n2):
            ...
            Energydf[str(iN)][Energydf[str(iN)] < 1.5] = 1e31
            Energydf[str(iN)][Energydf[str(iN)] > 10] = 1e31
         ...
         # screen for Kp
         kpIndex=np.where(np.array(Kp)>3)[0]
         Densitydf[kpIndex]=np.nan
  ...
  for iL in range(nLbins-1):
       for iMLT in range(nmlt_bins-1):
         ...
         # remove the nans
         try:
            sPden[iMLT][iL]=np.array(fplasmaDensity)[matches]
            sPden[iMLT][iL]=np.array(sPden[iMLT][iL])[~np.isnan(np.array(sPden[iMLT][iL]))]
         except(TypeError):
            sPden[iMLT][iL]=[]
            
     # save this file
  os.chdir('/Users/loisks/Desktop/ResearchProjects/PlasmaWaves/CorrectedDensity10eV')
  with open('1_10eV_plasma_density_'+str(n1)+'_'+str(n2)+'_sat='+satellite[iSat]+'_species='+spe[iSpe], 'wb') as f:    
     pickle.dump(sPden,f, protocol=2)
