...
mltbins=np.linspace(0.25, 23.75, 48)
sat=['A','B']
satellite=['a','b']
date=dateStart
...
for iFreq in range(len(freqIndex)):
  ...
  for iSat in range(len(sat)):
    E_Spectra_sorted=[[[] for i in range(nLbins)] for j in range(nmltbins)]
    ...
    while DT != endDt:
        ...
        if DT.month != monthCur:
            ...
            import pickling
            os.chdir('/Users/loisks/Desktop/ResearchProjects/PlasmaWaves/')
            pickling.hdf5_data_save(E_Spectra_sorted,'ESpectraMLT_sat=rbsp'+str(satellite[iSat])+'_'+str(yrCur)+'_'+str(monthCur) , 'WaveAmplitudes_'+str(freqLabel[iFreq])+'_Hz', nmltbins, nLbins)
